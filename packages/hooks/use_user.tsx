...
export const useProvideUser = (): useUserReturnType => {
  ...
  const useGetUserAcceptance = () => {
    ...
    return {
      loading,
      error,
      acceptance: data?.user?.status?.acceptance as Array<AcceptanceStatus>,
      refetch,
    };
  };
  ...
  const useCreateDwollaAccount = () => {
    const [createDwollaAccountInternal, { data, loading, error }] = useMutation(
  ...
  const useUserMetaData = () => {
    ...
    const firstName: string = data?.user?.firstName ? data.user.firstName : "";
