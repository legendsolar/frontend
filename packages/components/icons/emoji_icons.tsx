import { BaseIcon } from "../icons/base_icon";
import BankAccountIconPng from "../assets/icons/bank_account_icon.png";
import CalendarIconPng from "../assets/icons/calendar_icon.png";
import CashIconPng from "../assets/icons/cash_icon.png";
import EditFilledIconPng from "../assets/icons/edit_filled_icon.svg";
import EnvelopeIconPng from "../assets/icons/envelope_icon.png";
import HammerIconPng from "../assets/icons/hammer_icon.png";
import LeafIconPng from "../assets/icons/leaf_icon.png";
import MagGlassIconPng from "../assets/icons/mag_glass_icon.png";
import PhoneIconPng from "../assets/icons/phone_icon.png";
import PinIconPng from "../assets/icons/pin_icon.svg";
import PowerIconPng from "../assets/icons/power_icon.png";
import SunIconPng from "../assets/icons/sun_icon.png";
import UmbrellaIconPng from "../assets/icons/umbrella_icon.png";
import UserDataIconPng from "../assets/icons/user_data_icon.png";

import CupIconPng from "../assets/icons/cup.png";
import GearIconPng from "../assets/icons/gear.png";
import PushPinIconPng from "../assets/icons/push_pin.png";

export const CupIcon = ({ sx = {} }) => {
  return <BaseIcon src={CupIconPng} alt={"Cup Icon"} sx={sx}></BaseIcon>;
};

export const GearIcon = ({ sx = {} }) => {
  return <BaseIcon src={GearIconPng} alt={"Gear Icon"} sx={sx}></BaseIcon>;
};

export const PushPinIcon = ({ sx = {} }) => {
  return (
    <BaseIcon src={PushPinIconPng} alt={"Pin Push Icon"} sx={sx}></BaseIcon>
  );
};

export const BankIcon = ({ sx = {} }) => {
  return (
    <BaseIcon src={BankAccountIconPng} alt={"Bank Icon"} sx={sx}></BaseIcon>
  );
};

export const CalendarIcon = ({ sx = {} }) => {
  return <BaseIcon src={CalendarIconPng} alt={"Bank Icon"} sx={sx}></BaseIcon>;
};
export const CashIcon = ({ sx = {} }) => {
  return <BaseIcon src={CashIconPng} alt={"Bank Icon"} sx={sx}></BaseIcon>;
};
export const EditFilledIcon = ({ sx = {} }) => {
  return (
    <BaseIcon src={EditFilledIconPng} alt={"Bank Icon"} sx={sx}></BaseIcon>
  );
};
export const EnvelopeIcon = ({ sx = {} }) => {
  return <BaseIcon src={EnvelopeIconPng} alt={"Bank Icon"} sx={sx}></BaseIcon>;
};

export const HammerIcon = ({ sx = {} }) => {
  return <BaseIcon src={HammerIconPng} alt={"Bank Icon"} sx={sx}></BaseIcon>;
};
export const LeafIcon = ({ sx = {} }) => {
  return <BaseIcon src={LeafIconPng} alt={"Bank Icon"} sx={sx}></BaseIcon>;
};
export const MagGlassIcon = ({ sx = {} }) => {
  return <BaseIcon src={MagGlassIconPng} alt={"Bank Icon"} sx={sx}></BaseIcon>;
};
export const PhoneIcon = ({ sx = {} }) => {
  return <BaseIcon src={PhoneIconPng} alt={"Bank Icon"} sx={sx}></BaseIcon>;
};

export const PinIcon = ({ sx = {} }) => {
  return <BaseIcon src={PinIconPng} alt={"Bank Icon"} sx={sx}></BaseIcon>;
};
export const PowerIcon = ({ sx = {} }) => {
  return <BaseIcon src={PowerIconPng} alt={"Bank Icon"} sx={sx}></BaseIcon>;
};

export const SunIcon = ({ sx = {} }) => {
  return <BaseIcon src={SunIconPng} alt={"Bank Icon"} sx={sx}></BaseIcon>;
};

export const UmbrellaIcon = ({ sx = {} }) => {
  return <BaseIcon src={UmbrellaIconPng} alt={"Bank Icon"} sx={sx}></BaseIcon>;
};

export const UserDataIcon = ({ sx = {} }) => {
  return <BaseIcon src={UserDataIconPng} alt={"Bank Icon"} sx={sx}></BaseIcon>;
};
